...
const Home = () => {
  ...
  const [selectedHomePillCategory, setSelectedHomePillCategory] = useState('all');
  ...
  return (
    <div className="home-page" style={{ paddingTop: '0.5rem', paddingBottom: '6rem' }}>
      ...
      <section ref={recentProductsRef} className="section-container" style={{ margin: '0', padding: '0 16px' }}>
        ...
        {loading ? (
        ...
        ) : homeCategoryFilteredProducts.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px 20px', background: '#F8FAFC', borderRadius: '20px', border: '1px solid #E2E8F0', color: '#64748B' }}>
            <div style={{ fontSize: '32px', marginBottom: '8px' }}>🛍️</div>
            <div style={{ fontSize: '1rem', fontWeight: 800, color: '#0F172A' }}>Aucun produit dans cette catégorie pour le moment</div>
            <div style={{ fontSize: '0.85rem', marginTop: '4px' }}>Essayez de sélectionner une autre catégorie ou de réinitialiser les filtres.</div>
            <button
              onClick={() => setSelectedHomePillCategory('all')}
              style={{ marginTop: '12px', padding: '8px 16px', borderRadius: '12px', background: 'var(--primary)', color: 'white', border: 'none', fontWeight: 800, fontSize: '0.82rem', cursor: 'pointer' }}
            >
          ...
          <div className="grid grid-cols-2">
            {homeCategoryFilteredProducts.map((product) => {
            const imageUrl = product.images && product.images.length > 0 ? product.images[0] : '/hero.png';
